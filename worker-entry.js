...
async function getPost(env,id){
  const p=await env.DB.prepare(`SELECT p.*,c.name category_name,c.slug category_slug,COALESCE(NULLIF(m.url,''),NULLIF(p.social_image,'')) cover_url,m.alt_text cover_alt,m.caption cover_caption,u.display_name author_name
...
async function page(env,request,url){
  ...
  if(path.match(/^\/post\/[^/]+$/)){
    ...
    const p=await env.DB.prepare("SELECT p.title,p.slug,p.excerpt,p.content,p.meta_title,p.meta_description,p.social_image,p.published_at,p.updated_at,p.type,c.name category_name,c.slug category_slug,COALESCE(NULLIF(m.url,''),NULLIF(p.social_image,'')) cover_url,m.width cover_width,m.height cover_height,m.alt_text cover_alt,u.display_name author_name,t.title translation_title,t.excerpt translation_excerpt,t.meta_title translation_meta_title,t.meta_description translation_meta_description FROM posts p LEFT JOIN categories c ON c.id=p.category_id LEFT JOIN media m ON m.id=p.cover_media_id LEFT JOIN users u ON u.id=p.author_id LEFT JOIN post_translations t ON t.post_id=p.id AND t.language=? WHERE p.slug=? AND p.status='published' LIMIT 1").bind(lang,slug).first();
    if(!p)return asset;
