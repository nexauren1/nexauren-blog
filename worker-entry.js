...
async function api(env,request,url){
  ...
  if(p==="/api/settings"&&m==="GET"){const r=await env.DB.prepare("SELECT key,value,type FROM settings ORDER BY key").all(),s={};for(const x of r.results)s[x.key]=x.value;return json({ok:true,settings:s});}
