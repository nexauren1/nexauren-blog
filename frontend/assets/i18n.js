...
function translateTextNodes(){
 ...
 for(const n of nodes){if(shouldSkip(n))continue;const raw=n.nodeValue||"",key=raw.trim();if(!key)continue;const translated=serverMap.get(key)||PT[key];if(translated&&translated!==key){n.nodeValue=raw.replace(key,translated)}}
}
function translateAttrs(){
 if(lang!=="en")return;
 document.querySelectorAll("input[placeholder],textarea[placeholder],[aria-label],title").forEach(el=>{
   for(const attr of ["placeholder","aria-label","title"]){if(!el.hasAttribute(attr))continue;const v=el.getAttribute(attr)||"",tr=serverMap.get(v)||ATTR_MAP[v]||PT[v];if(tr&&tr!==v)el.setAttribute(attr,tr)}
 });
}
