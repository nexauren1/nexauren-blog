...
async function home(){
  app.innerHTML='<section class="hero"><div class="hero-copy"><div class="eyebrow">'+t("official")+'</div><h1>'+ (lang==="en"?"The story behind everything Nexauren.":"A história por trás de tudo o que é Nexauren.") +'</h1><p>'+(lang==="en"?"News, launches, app updates, guides, tutorials and ideas shaping the ecosystem.":"Notícias, lançamentos, atualizações, guias, tutoriais e ideias que moldam o ecossistema.")+'</p></div><div class="hero-card"><div class="hero-orbit"><span>N</span></div><div><div class="eyebrow">NEXAUREN STORY</div><p style="color:var(--muted)">'+(lang==="en"?"One place for the latest from the ecosystem.":"Um só lugar para acompanhar as novidades do ecossistema.")+'</p></div></div></section><section class="section"><div class="section-head"><div><h2>'+t("explore")+'</h2><p>'+t("exploreDesc")+'</p></div></div><div id="cat-grid" class="category-grid"></div></section><section class="section" id="featured"></section><section class="section"><div class="section-head"><div><h2>'+t("latest")+'</h2><p>'+t("latestDesc")+'</p></div><a class="read-more" href="/posts">'+t("viewAll")+'</a></div><div id="latest" class="grid"></div></section>';
