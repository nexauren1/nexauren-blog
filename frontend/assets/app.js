...
const DEFAULT_SOCIAL_IMAGE=location.origin+"/assets/social-preview-nexauren.png?v=20260926-2";
...
async function wireComments(postId,allowComments){
  ...
  form.addEventListener("submit",async e=>{e.preventDefault();feedback.textContent="";const body=document.getElementById("comment-body")?.value.trim()||"",parent=document.getElementById("comment-parent")?.value||"";if(!body)return feedback.textContent=lang==="en"?"Write a comment.":"Escreva um comentário.";const submit=form.querySelector("button[type=submit]");submit.disabled=true;try{const d=await firebaseWorkerFetch("/api/comments?post_id="+encodeURIComponent(postId),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({body,parent_id:parent||null})});if(!d){location.href="/account?return="+encodeURIComponent(location.href);return;}document.getElementById("comment-body").value="";document.getElementById("comment-parent").value="";document.getElementById("comment-replying").textContent="";feedback.textContent=d.moderated?(lang==="en"?"Comment submitted for review.":"Comentário enviado para aprovação."): (lang==="en"?"Comment published.":"Comentário publicado.");await loadComments(postId,list,form,allowComments);}catch(x){feedback.textContent=x.message||"Não foi possível publicar."; }finally{submit.disabled=false;}});
  list.addEventListener("click",async e=>{
    ...
    if(e.target.closest("[data-comment-reply]")){if(list.dataset.allowReplies!=="1")return;document.getElementById("comment-parent").value=id;document.getElementById("comment-replying").textContent=(lang==="en"?"Replying to ":"A responder a ")+(item.querySelector(".blog-comment-head strong")?.textContent||"comentário");document.getElementById("comment-body").focus();return;}
  ...
  document.getElementById("comment-cancel").onclick=()=>{document.getElementById("comment-parent").value="";document.getElementById("comment-replying").textContent="";};
}
async function post(slug){
  ...
  try{
    ...
    updateSeo({title:p.meta_title||p.title,description,type:"article",image:new URL(p.cover_url||p.social_image||"/assets/social-preview-nexauren.png?v=20260926-2",location.origin).href,url:canonicalUrl,hasEnglish:p.translation_available,robots:(lang==="en"&&!p.translation_available)?"noindex,follow":undefined});
