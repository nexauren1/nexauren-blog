(() => {
  ...
  function economics(price){
    ...
    const marketplaceFeePerUnit=customerTotal*marketplace;
    const paymentFeePerUnit=customerTotal*payment;
    const adsFeePerUnit=customerTotal*ads;
    const percentFees=customerTotal*feeRate;
    const feesPerUnit=percentFees+fixedFee/qty;
    const product=num(fields.productCost), shipping=num(fields.shippingCost), other=num(fields.otherCosts);
    const operatingPerUnit=product+shipping+other;
    const profitPerUnit=netRevenueBeforeFees-operatingPerUnit-feesPerUnit;
    const totalRevenue=netRevenueBeforeFees*qty;
    const totalTax=taxPerUnit*qty;
    const totalPercentFees=percentFees*qty;
    const totalFees=totalPercentFees+fixedFee;
    const totalVariableCosts=(operatingPerUnit*qty)+totalFees;
    ...
    return {discountedPrice,customerTotal,netRevenueBeforeFees,taxPerUnit,marketplaceFeePerUnit,paymentFeePerUnit,adsFeePerUnit,percentFees,feesPerUnit,product,shipping,other,operatingPerUnit,profitPerUnit,totalRevenue,totalTax,totalPercentFees,totalFees,totalVariableCosts,totalProfit,margin,totalCosts,roi,feeRate,taxRate:tax,discountRate:discount};
  }
  function requiredPrice(target="breakEven"){
    ...
    const unitBase=num(fields.productCost)+num(fields.shippingCost)+num(fields.otherCosts)+num(fields.fixedFee)/qty+num(fields.fixedCosts)/qty;
    ...
    if(target==="profit"){const perUnitTarget=num(fields.targetProfit)/qty;return (unitBase+perUnitTarget)/profitCoeff}
    return 0;
  }
  ...
  function calculate(){
    ...
    document.getElementById("unitProfit").textContent=money(e.profitPerUnit-num(fields.fixedCosts)/qty);
    document.getElementById("unitTotalCost").textContent=money(e.operatingPerUnit+e.feesPerUnit+num(fields.fixedCosts)/qty);
    ...
    document.getElementById("unitFees").textContent=money(e.feesPerUnit);
    document.getElementById("breakdown").innerHTML=[
      ...
      row("Taxa do marketplace",e.marketplaceFeePerUnit*qty,"fee"),
      row("Taxa de pagamento",e.paymentFeePerUnit*qty,"fee"),
      row("Publicidade / anúncios",e.adsFeePerUnit*qty,"fee"),
      row("Comissão fixa por venda",num(fields.fixedFee),"fee"),
    ...
    document.getElementById("minimumPrice").textContent=displayPrice(requiredPrice("breakEven"));
    document.getElementById("marginPrice").textContent=displayPrice(requiredPrice("margin"));
    document.getElementById("profitPrice").textContent=num(fields.targetProfit)>0?displayPrice(requiredPrice("profit")):"—";
