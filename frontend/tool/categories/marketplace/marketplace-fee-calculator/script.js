(() => {
  ...
  const locales={EUR:"de-DE",USD:"en-US",GBP:"en-GB",MZN:"pt-MZ",ZAR:"en-ZA",BRL:"pt-BR",CAD:"en-CA",AUD:"en-AU",CHF:"de-CH",JPY:"ja-JP",CNY:"zh-CN",INR:"en-IN",NGN:"en-NG",KES:"en-KE",GHS:"en-GH",AED:"ar-AE"};
  ...
  function money(value){const code=fields.currency.value||"EUR";const amount=Number.isFinite(value)?value:0;const sign=amount<0?"−":"";const digits=zeroDecimal.has(code)?0:2;try{return sign+new Intl.NumberFormat(locales[code]||"en-US",{style:"currency",currency:code,minimumFractionDigits:digits,maximumFractionDigits:digits}).format(Math.abs(amount))}catch{return sign+(symbols[code]||code)+" "+Math.abs(amount).toFixed(digits)}}
  function pct(value){const amount=Number.isFinite(value)?value:0;return `${amount<0?"−":""}${Math.abs(amount).toFixed(2)}%`}
  ...
  function economics(price){
    const qty=quantity(),discount=rate(fields.discount),tax=rate(fields.taxRate),marketplace=rate(fields.marketplaceFee),payment=rate(fields.paymentFee),ads=rate(fields.adsFee),fixedFee=num(fields.fixedFee);
    const discountedPrice=price*(1-discount);let customerTotal,netRevenueBeforeFees,taxPerUnit;
    ...
    const marketplaceFeePerUnit=customerTotal*marketplace,paymentFeePerUnit=customerTotal*payment,adsFeePerUnit=customerTotal*ads,percentFees=customerTotal*(marketplace+payment+ads),feesPerUnit=percentFees+fixedFee/qty;
    const product=num(fields.productCost),shipping=num(fields.shippingCost),other=num(fields.otherCosts),operatingPerUnit=product+shipping+other,profitPerUnit=netRevenueBeforeFees-operatingPerUnit-feesPerUnit;
    const totalRevenue=netRevenueBeforeFees*qty,totalTax=taxPerUnit*qty,totalFees=percentFees*qty+fixedFee,totalVariableCosts=operatingPerUnit*qty+totalFees,totalProfit=totalRevenue-totalVariableCosts-num(fields.fixedCosts),margin=totalRevenue>0?totalProfit/totalRevenue*100:0,totalCosts=totalRevenue-totalProfit,roi=totalCosts>0?totalProfit/totalCosts*100:0;
  ...
  function requiredPrice(target){
    const qty=quantity(),d=rate(fields.discount),t=rate(fields.taxRate),f=rate(fields.marketplaceFee)+rate(fields.paymentFee)+rate(fields.adsFee);let revenueCoeff,feeCoeff;
    if(fields.taxMode.value==="added"){revenueCoeff=1-d;feeCoeff=(1-d)*(1+t)*f}else{revenueCoeff=(1-d)/(1+t);feeCoeff=(1-d)*f}
    const profitCoeff=revenueCoeff-feeCoeff,unitBase=num(fields.productCost)+num(fields.shippingCost)+num(fields.otherCosts)+num(fields.fixedFee)/qty+num(fields.fixedCosts)/qty;if(profitCoeff<=0)return Infinity;
    if(target==="breakEven")return unitBase/profitCoeff;if(target==="margin"){const margin=rate(fields.targetMargin),denominator=profitCoeff-margin*revenueCoeff;return denominator>0?unitBase/denominator:Infinity}if(target==="profit")return(unitBase+num(fields.targetProfit)/qty)/profitCoeff;return 0;
  }
  ...
  function calculate(){
    ...
    document.getElementById("netProfit").textContent=money(e.totalProfit);document.getElementById("margin").textContent=pct(e.margin);document.getElementById("unitProfit").textContent=money(e.profitPerUnit-num(fields.fixedCosts)/qty);document.getElementById("unitTotalCost").textContent=money(e.operatingPerUnit+e.feesPerUnit+num(fields.fixedCosts)/qty);document.getElementById("unitRevenue").textContent=money(e.netRevenueBeforeFees);
    ...
    document.getElementById("minimumPrice").textContent=displayPrice(requiredPrice("breakEven"));document.getElementById("marginPrice").textContent=displayPrice(requiredPrice("margin"));document.getElementById("profitPrice").textContent=num(fields.targetProfit)>0?displayPrice(requiredPrice("profit")):"—";document.getElementById("contributionUnit").textContent=money(e.profitPerUnit);document.getElementById("breakEvenUnits").textContent=e.profitPerUnit>0?units(Math.max(0,Math.ceil(num(fields.fixedCosts)/e.profitPerUnit))):"Não atingível";document.getElementById("scenarios").innerHTML=[scenarioCard("−10%",price*.9),scenarioCard("Preço atual",price,true),scenarioCard("+10%",price*1.1)].join("");
  ...
  Object.values(fields).forEach(el=>{el.addEventListener("input",calculate);el.addEventListener("change",()=>{updatePrefixes();calculate()})});document.getElementById("reset").addEventListener("click",reset);updatePrefixes();calculate();
})();