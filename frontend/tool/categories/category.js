(() => {
  const DATA_URL="/tool/data/data.json";
  ...
  async function loadRegistry(){
    if(!registryPromise) registryPromise=fetch(DATA_URL,{cache:"no-store"}).then(r=>{if(!r.ok)throw new Error("Não foi possível carregar o catálogo de ferramentas.");return r.json()}).then(normalize);
