...
function applyLanguage(){
 ...
 set("#pdfsplit-lead",x.lead);set("#pdfsplit-radar-label",x.local);set("#pdfsplit-drop-title",x.drop);set("#pdfsplit-drop-sub",x.sub);set("#pdfsplit-file-label",x.file);
 ...
 set("#pdfsplit-flow1",x.flow1);set("#pdfsplit-flow2",x.flow2);set("#pdfsplit-flow3",x.flow3);set("#pdfsplit-lang",x.translate);set(".pdfsplit-back",x.back);
 renderResults();
}
function renderResults(){
 ...
 outputs.forEach((r,i)=>{const row=document.createElement("div");row.className="pdfsplit-result";const idx=document.createElement("div");idx.className="pdfsplit-result-index";idx.textContent=String(i+1).padStart(2,"0");const info=document.createElement("div");const b=document.createElement("strong");b.textContent=r.name;const s=document.createElement("small");s.textContent=t().result+" · "+r.pages+" · "+bytes(r.size);info.append(b,s);const btn=document.createElement("button");btn.className="pdfsplit-btn primary";btn.type="button";btn.textContent=t().download;btn.addEventListener("click",()=>download(r.blob,r.name));row.append(idx,info,btn);results.append(row)})
}
...
function parseRange(value,max){
 ...
 for(const raw of String(value||"").split(",")){
  ...
  const m=s.match(/^(\d+)(?:\s*-\s*(\d+))?$/);if(!m)throw Error(t().invalidRange);
  const a=Number(m[1]),b=m[2]?Number(m[2]):a;
  if(a<1||b<1||a>max||b>max||a>b)throw Error(t().outside);
...
function loadScript(src){
 return new Promise((resolve,reject)=>{const s=document.createElement("script");s.src=src;s.async=true;s.onload=()=>window.PDFLib?.PDFDocument?resolve(window.PDFLib):reject(Error(t().engine));s.onerror=()=>reject(Error(t().engine));document.head.appendChild(s)})
}
...
async function openFile(f){
 ...
 file=f;pdf=null;outputs=[];fileBar.hidden=false;fileName.textContent=f.name;fileMeta.textContent=bytes(f.size);total.textContent="…";parts.textContent="0";state.textContent=t().loading;renderResults();splitBtn.disabled=true;msg(t().loading+"…");
 try{
  const lib=await ensureEngine();pdf=await lib.PDFDocument.load(await f.arrayBuffer(),{ignoreEncryption:true});
  total.textContent=String(pdf.getPageCount());parts.textContent=mode.value==="each"?String(pdf.getPageCount()):"1";state.textContent=t().ready;splitBtn.disabled=false;msg(t().ready);
...
async function split(){
 ...
 try{
  const lib=await ensureEngine(),pages=selectedPages(pdf.getPageCount()),base=(nameInput.value.trim().replace(/[^a-z0-9_-]+/gi,"-").replace(/^-+|-+$/g,"")||"parte");
  for(let i=0;i<pages.length;i++){
   msg(t().processing+" "+(i+1)+"/"+pages.length+"…");
   const out=await lib.PDFDocument.create(),copied=await out.copyPages(pdf,pages[i].map(n=>n-1));copied.forEach(p=>out.addPage(p));
   const bytesOut=await out.save({useObjectStreams:false}),blob=new Blob([bytesOut],{type:"application/pdf"});
   const suffix=pages.length===1?base:base+"-"+String(i+1).padStart(2,"0");
   outputs.push({blob,name:suffix+".pdf",pages:pages[i].length,size:blob.size});renderResults();
   progress.style.width=Math.round((i+1)/pages.length*100)+"%";parts.textContent=String(pages.length);state.textContent=t().processing;await new Promise(requestAnimationFrame);
  }
