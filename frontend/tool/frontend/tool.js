(() => {
  ...
  function toolCard(t){
    ...
    const badges=(t.featured?'<span class="tool-card-badge featured">Destaque</span>':"")+
      (t.popular?'<span class="tool-card-badge popular">Popular</span>':"")+
      (locked?'<span class="tool-card-badge pro">PRO</span>':"");
    return '<a class="tool-card cat-'+esc(t.category||"geral")+(locked?" tool-card-locked":"")+' nx-spotlight nx-reveal" data-tool-id="'+esc(t.id)+'" data-tool-access="'+esc(t.access||"public")+'" data-tool-path="'+esc(t.path)+'" href="'+esc(t.path)+'">'+
      '<div class="tool-card-top"><span class="tool-card-icon">'+iconSvg(t.category)+'</span><span class="tool-arrow">→</span></div>'+
      '<div class="tool-card-badges">'+badges+'</div><h2>'+esc(t.name)+'</h2><p>'+esc(t.description||"")+'</p><small>'+esc((t.tags||[]).slice(0,4).join(" · "))+'</small>'+
      (locked?'<span class="tool-lock" aria-hidden="true">🔒</span>':"")+'</a>';
  }

  const categoryCard=c=>'<a class="tool-card category-card cat-'+esc(c.id)+' nx-spotlight nx-reveal" href="'+esc(c.path||("/tool/categories/"+c.id+"/"))+'">'+
  ...
  function renderAccessMessage(message){
    ...
    el.innerHTML='<div class="tool-access-dialog-backdrop" data-tool-access-close></div>'+
      '<div class="tool-access-dialog-panel" role="dialog" aria-modal="true" aria-labelledby="tool-access-title">'+
      '<button class="tool-access-dialog-close" type="button" data-tool-access-close aria-label="Fechar">×</button>'+
      '<span class="tool-access-dialog-icon">🔒</span><div class="tool-eyebrow">NEXAUREN PRO</div>'+
      '<h3 id="tool-access-title">'+esc(message.title)+'</h3><p>'+esc(message.body)+'</p>'+
      '<div class="tool-access-dialog-actions"><a class="primary" href="'+esc(message.actionUrl||"/account")+'">'+esc(message.actionLabel||"Continuar")+'</a><button class="secondary" type="button" data-tool-access-close>Agora não</button></div></div>';
  ...
  function updateSearch(){
    ...
    if(q)localStorage.setItem("nexauren-tool-search",String(search.value).slice(0,100));else localStorage.removeItem("nexauren-tool-search");
  ...
  async function handleToolClick(event){
    ...
    try{
      ...
      renderAccessMessage({
        ...
        actionLabel:"Ir para o plano Pro",
        actionUrl:accessApi.upgradeUrl()
      });
  ...
  async function loadCatalog(){
    ...
    try{
      const registryPromise=window.NexaurenToolRegistry.loadRegistry();
      const planPromise=accessApi.getPlanState({force:false});
      const [registry,plan]=await Promise.all([registryPromise,planPromise]);
      applyRegistry(registry);isPro=plan.pro;
      updateSearch();
    }catch(e){
      if(!categories.length)grid.innerHTML='<div class="tool-empty">'+esc(e.message||"Não foi possível carregar o catálogo.")+'</div>';
    }
  }
  ...
  function applyRegistry(registry){
    ...
    if(search&&stored)updateSearch();else{renderCategories();renderResults([])}
  }

  async function refreshPlan(){
    if(!user)return;
    try{const plan=await accessApi.getPlanState({force:true});isPro=plan.pro;updateSearch()}catch{}
  }

  document.addEventListener("click",handleToolClick,true);
  search?.addEventListener("input",updateSearch);
  window.addEventListener("nexauren:tool-registry-updated",event=>{if(user&&event.detail)applyRegistry(event.detail)});
  ...
  (async()=>{
    try{
      ...
      accessApi.onAuthStateChanged(accessApi.auth,async currentUser=>{
        user=currentUser;
        if(!user){isPro=null;categories=[];tools=[];renderAccountGate();return}
        await loadCatalog();
        refreshPlan();
      });
    }catch(e){
      if(grid)grid.innerHTML='<div class="tool-empty">Não foi possível iniciar a autenticação das ferramentas.</div>';
    }
  })();
})();