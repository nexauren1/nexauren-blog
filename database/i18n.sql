-- Nexauren i18n — instalação manual no D1 nexauren-blog.
-- Não é uma migration. Execute este ficheiro manualmente quando quiser ativar o armazenamento de traduções.
...
CREATE TABLE IF NOT EXISTS translation_state (
  ...
  language TEXT NOT NULL CHECK (language IN ('pt','en')),
  source_hash TEXT NOT NULL DEFAULT '',
  status TEXT NOT NULL DEFAULT 'translated' CHECK (status IN ('pending','translated','approved','stale','error')),
  error_message TEXT NOT NULL DEFAULT '',
  updated_at TEXT NOT NULL,
